...
export {
    sumTo, factorial, fibonacci, outputListLoop, outputList, reverseList, reverseListLoop, sumTreeValues
}
...
// Using iterator
function outputListLoop(list: List): string {
    let result = "";
    let current = list;
    while (current !== null) {
        // Get the value
...
function reverseList(list: List): string {
    // Base case: Check if the first node is empty
    if (list === null) {
        return "";
    }
...
function reverseListLoop(list: List): string {
    let result = "";
    let current = list;
    while (current !== null) {
        // Get the value
...
function sumTreeValues(node: TreeNode | null): number {
    // Base case
    if(node === null){
        return 0;
    }
